...
- **Where the key is sent.** Requests go to the configured z.ai endpoint:
  ...
  point it at endpoints you trust. A set value that names no endpoint (empty, only slashes
  or whitespace) is refused — the server does not start — rather than replaced with the
  z.ai default: the variable is how egress is scoped, and it is never silently unscoped.
