...
function delegate({ code = 0, stdout = resultLine({}), note = '', task = 'do the thing',
  ...
  if (git) {
    ...
    // #77's branch shapes, inside #93's conditional: a run with no git base
    // has no branch to check out and must not try.
    if (onBranch) g('checkout', '-q', '-b', onBranch);
    if (detach) g('checkout', '-q', '--detach');
...
test('a "#n" that names a pull request records no issue: PRs and issues share one number sequence', () => {
  // writes+commit: this asserts the run completes normally, and since #93 a
  // delegation that commits nothing does not — correctly. The worker has to do
  // real work for "exit 0" to be a statement about the issue field.
  const r = delegate({ task: 'Review PR #45, then merge it', writes: 'w', commit: true });
...
test('an in-place run on a detached HEAD records branch as present-and-null, not a crash or a SHA', () => {
  const r = delegate({ detach: true, writes: 'w', commit: true });
